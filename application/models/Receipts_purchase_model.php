...
class Receipts_purchase_model extends CI_Model
{
    var $table = "receipts_purchase";
    ...
    {
        $next_count = $this->settings_model->SYS_Settings->purchase_receipt_next;
    ...
    public function getReceiptsEmails ($id_list = array())
    {
        $q = $this->db
                ->select("email")
                ->join("suppliers", "suppliers.id=receipts_purchase.purchase_id", "left")
                ->where_in("receipts_purchase.id", $id_list)
                ->group_by("email")
                ->get ( $this->table );
        if ( $q->num_rows () > 0 ) {
            $result = array();
