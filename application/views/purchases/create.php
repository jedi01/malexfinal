...
$purchase_items = set_value("purchase_item", "");
if( set_value("purchase_item", "") != "" ){
    $purchase_items = set_value("purchase_item", "");
}
...
echo form_input(array("type"=>"hidden", "name"=>"purchase[count]","value"=>$next_count,"id"=>"next_count"));
...
<ol class="breadcrumb pos-sticky">
    ...
    <div class="flip pull-right" style="line-height: 64px;">
        ...
        <a href="#" class="btn btn-link btn-sm preview_purchase" id="preview" >
		...
		<div class="card">
            <div class="card-header">
                <div class="col-sm-4 form-group m-a-0">
                    <?php echo form_input('purchase[title]', set_value('purchase[title]', lang("purchase")), $purchase_title); ?>
                </div>
                <div class="col-md-4 form-group m-a-0">
                    ...
                    foreach ($this->settings_model->getFormattedCurrencies() as $currency) {
                        echo "<option value='".$currency->value."' symbol_native='".$currency->symbol_native."' ".($currency->value==set_value("purchase[currency]", $inv_currency)?"selected='selected'":"" ).">".$currency->label."</option>";
                    }
                    echo "</select>";
                    ?>
                </div>
                <div class="col-sm-4 form-group m-a-0">
                    <?php
                    $status = $this->settings_model->getStatus();
                    unset($status['canceled']);
                    unset($status['panding']);
                    unset($status['overdue']);
                    $default_status = DEFAULT_STATUS;
                    echo form_dropdown('purchase[status]', $status, set_value("purchase[status]", $default_status), $purchase_status);
                    ?>
                </div>
			...
			<div class="card-block">
                <div class="col-md-6">
                    ...
                    <div class="form-group row required">
                        ...
                        <div class="col-md-9">
                            <div class="input-group">
                                ...
                                <?php echo form_input('purchase[reference]', set_value('purchase[reference]', $next_reference), $purchase_reference); ?>
                    ...
                    <div class="form-group row required">
                        ...
                        <?php echo form_hidden('purchase[supplier_id]', set_value('purchase[supplier_id]', $_supplier_id)); ?>
                        <div class="col-md-9">
                            <div class="input-group">
                                <span class="input-group-addon"><i class="fa fa-user"></i></span>
                                <input type="text" class="form-control" id="supplier_id" value="<?php echo set_value("biller",$_supplier_name) ?>" name="supplier_id">
                                <span class="input-group-btn">
                    ...
                    <div class="form-group row required">
                        ...
                        <div class="col-md-9">
                            <div class="input-group">
                                ...
                                echo form_hidden('purchase[date]', set_value('purchase[date]', date_MYSQL_JS(date("Y-m-d"))));
                                ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            ...
                            <div class="input-group m-t-1">
                                ...
                                echo form_hidden('purchase[date_due]', set_value('purchase[date_due]', ""));
                                ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="col-md-12">
                    <div class="form-group row">
                        <div class="col-md-12">
                            <label class="form-control-label"><?php echo lang("purchase_notes"); ?></label>
                            <textarea class="form-control" rows="3" name="purchase[note]" id="editor_note"><?php echo set_value("purchase[note]", $_note) ?></textarea>
                        </div>
                    </div>
                </div>
                <div class="clearfix"></div>
            ...
            <div class="card-footer text-md-right">
                <a href="<?php echo site_url("/purchase") ?>" class="btn btn-secondary"><?php echo lang("cancel") ?></a>
                <a href="#" class="btn btn-secondary preview_purchase" id="preview"><i class="fa fa-print"></i> <?php echo lang("preview") ?></a>
...
$(document).ready(function() {
    ...
    function get_next_reference(refresh){
        ...
        if( refresh ){
            ...
            switch(reference_type){
                ...
                case 3: ref_mask = "9?99999"+PURCHASE_PREFIX; ref_placeholder = "______"+PURCHASE_PREFIX; break;
     ...
     *  BILLER (AUTOCOMPLETE)
     */
    var selected_biller = <?php echo json_encode($supplier_js); ?>;
    $('#supplier_id')
    .change(function(){
        if( $(this).val() == "" ){
    ...
    .easyAutocomplete({
        ...
        template: {
            ...
            method: function(value, item) {
                ...
                actions += "<a href='#' class='delete_supplier btn btn-sm btn-secondary' data-id='" + item.id + "'><i class='fa fa-trash'></i></a>";
    ...
    $('.preview_purchase').click(function(){
        var data = $('#form').serialize();
        if( $('input[name="purchase[date]"]').val() == ""
            || $('input[name="purchase[supplier_id]"]').val() == "" ){
            showToastr("error", globalLang["preview_purchase_error"]);
            return false;
        }
        $.ajax({
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#every').on("change", function(){
            ...
            if( frequency == "weekly" ){
                text += globalLang["on"]+" <label for='date'><b>"+date.format("dddd")+"</b></label> ";
            }else if( frequency == "monthly" ){
                text += globalLang["on_the"]+" <label for='date'><b>"+date.format("Do")+"</b></label> ";
            }else if( frequency == "yearly" ){
                text += globalLang["on_the"]+" <label for='date'><b>"+date.format("Do, MMMM")+"</b></label> ";
            }
            if( occurences == 0 ){
            ...
            $("#recurring_end").html(globalLang["recurring_effective"] +" <label for='date'><b>"+date.format(JS_DATE)+"</b></label>");
        });
