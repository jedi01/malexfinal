...
<ol class="breadcrumb">
    ...
    <div class="flip pull-right" style="line-height: 64px;">
        ...
        <a href="<?php echo site_url('/purchase_receipts/create/'.$purchase);?>" sis-modal="receipts_table" class="btn btn-primary-outline sis_modal tip" title="<?php echo lang("receipts_create"); ?>"> <i class="fa fa-plus"></i></a>
        <?php endif ?>
        ...
        <div class="btn-group tip" title="<?php echo lang("filter"); ?>">
            ...
            <ul class="dropdown-menu dropdown-menu-right">
                ...
                <a href="<?php echo site_url('/purchase_receipts');?>" class="dropdown-item <?php echo $class ?>"><?php echo lang("all") ?></a>
                <?php foreach ($this->settings_model->getPaymentsMethods() as $method => $label): ?>
                    <?php $class = (isset($_GET['fv']) && $_GET['fv'] == $method)?"active":""; ?>
                    <a href="<?php echo site_url('/purchase_receipts?f=method&fv='.$method);?>" class="dropdown-item <?php echo $class ?>"><?php echo $label ?></a>
                <?php endforeach ?>
            </ul>
        </div>
...
$(document).ready(function() {
    ...
    var receipts_table = $('#receipts_table').dataTable( {
        ...
        'bProcessing'    : true,
        'bServerSide'    : true,
        'sAjaxSource'    : SITE_URL+'/purchase_receipts/getData',
        'fnServerData': function(sSource, aoData, fnCallback)
        {
            ...
            if( purchase != undefined ){
                aoData.push( { "name": "purchase", "value": purchase } );
            }
            <?php
            if( isset($biller_id)){
    ...
    $(document).on('click', '.pdf_selected', function() {
        if( !$(this).is('.disabled') ){
            ...
            MyWindow=window.open(SITE_URL+"/purchase_receipts/pdf?id="+selected_rows.join(","), WINDDOW_NAME, WINDDOW_CONFIGURATION);
        }
        return false;
    });
    ...
    function actions_format(data, type, row, meta){
        actions = '<div class="btn-group">'+
                    ...
                    '<ul class="dropdown-menu dropdown-menu-right">';
        <?php if (!$this->ion_auth->in_group(array("customer", "supplier"))): ?>
        actions += $('#receipts_table').create_datatable_action("trash", SITE_URL+"/purchase_receipts/delete?id="+row.id, globalLang['delete'], false, true, false, true);
        actions += $('#receipts_table').create_datatable_action("pencil", SITE_URL+"/purchase_receipts/edit/"+row.id, globalLang['edit'], false, false, true);
        actions += '<li class="dropdown-divider" ></li>';
        <?php endif ?>
        actions += $('#receipts_table').create_datatable_action("print", SITE_URL+"/purchase_receipts/view/"+row.id, globalLang['print'], true);
        actions += $('#receipts_table').create_datatable_action("download", SITE_URL+"/purchase_receipts/pdf/"+row.id, globalLang['tabletool_pdf'], false);
        <?php if (!$this->ion_auth->in_group(array("customer", "supplier"))): ?>
        actions += $('#receipts_table').create_datatable_action("envelope", SITE_URL+"/purchase_receipts/email/"+row.id, globalLang['email'], false, false, true);
        <?php endif ?>
        actions += '</ul></div>';
        return "<center>"+actions+"</center>";
    }
    function reference_format(value, y ,row) {
        var html = '<a href="'+SITE_URL+'/purchases/open/'+row.purchase_id+'"><small class="font-weight-bold">'+value+'</small></a>';
        return  html;
    }
    function number_format(data, type, row, meta) {
        var value = "<?php echo RECEIPT_PREFIX ?>"+String("00000" + data).slice(-5);
        var html = '<a href="#" onclick="MyWindow=window.open(\'<?php echo site_url("/purchase_receipts/view/'+row.id+'") ?>\', WINDDOW_NAME,WINDDOW_CONFIGURATION); return false;"><small class="font-weight-bold">'+value+'</small></a>';
        return  html;
    }
