...
<div class="breadcrumb">
	...
	<div class="flip pull-right" style="line-height: 64px;">
        ...
        <a href="<?php echo site_url('/invoices/create');?>" class="btn btn-primary-outline tip" title="<?php echo lang("create_invoice"); ?>"> <i class="fa fa-plus"></i></a>
        <?php endif ?>
        ...
        <div class="btn-group tip" title="<?php echo lang("filter"); ?>">
            ...
            <ul class="dropdown-menu dropdown-menu-right">
                ...
                <a href="<?php echo site_url('/invoices');?>" class="dropdown-item <?php echo $class ?>"><?php echo lang("all") ?></a>
                <?php foreach ($this->settings_model->getInvoiceStatus() as $status => $label): ?>
                    <?php $class = (isset($_GET['fv']) && $_GET['fv'] == $status)?"active":""; ?>
                    <a href="<?php echo site_url('/invoices?f=status&fv='.$status);?>" class="dropdown-item <?php echo $class ?>"><?php echo $label ?></a>
                <?php endforeach ?>
            </ul>
        </div>
		<div class="btn-group download-list tip" title="<?php echo lang("tabletool_collection"); ?>" export-table="<?php echo $invoices_table ?>">
...
<?php endif ?>
				<table id="<?php echo $invoices_table ?>" class="table table-sm table-hover serverSide checkable_datatable">

                    <style type="text/css">
                        #<?php echo $invoices_table ?> th{
                            font-weight: bold;
                        }
                    </style>
					<thead>
...
$(document).ready(function() {
    ...
    ?>
    <?php echo $invoices_table ?> = $('#<?php echo $invoices_table ?>').dataTable( {
        "oColVis": {
            "aiExclude": [0, 1, 4, 14],
        },
        "aoColumnDefs": [{
            "bVisible": false,
            "aTargets": [2,3,5,10,11,12,13] ,
        }],
        "aaSorting": [[ 1, "desc" ]],
        'bProcessing'    : true,
        'bServerSide'    : true,
        "bSortCellsTop"  : true,
        'sAjaxSource'    : SITE_URL+'/invoices/getdata',
        ...
        'fnDrawCallback': function(){
            $.each($(this).find('a.biller_popover'), function(i, biller_popover){
                $(biller_popover).popover({
                    ...
                    content:'<ul>'+
                    '<li class="dropdown-item"><a href="'+SITE_URL+'/billers/view?id='+$(biller_popover).data("bill_to_id")+'" class="text-inherit sis_modal" sis-modal="<?php echo $invoices_table ?>"><i class="fa fa-expand"></i>'+globalLang["details"]+'</a></li>'+
            ...
            $.each($(this).find('a.status_popover'), function(i, status_popover){
                $(status_popover).popover({
                    ...
                    template: '<div class="popover"><div class="arrow"></div><div class="popover-content"></div></div>',
                    content:'<ul>'+
                    '<li class="dropdown-item"><a href="'+SITE_URL+'/invoices/set_status?id='+$(status_popover).data("id")+'" class="text-inherit sis_modal" sis-modal="<?php echo $invoices_table ?>"><i class="fa fa-pencil"></i>'+globalLang["edit"]+'</a></li>'+
        ...
        "aoColumns": [
            ...
            { "sName": 'payment_date', "mDataProp": 'payment_date', "mRender": date_due_format},
            { "sName": 'total'    , "mDataProp": 'total'    , "mRender": prices_format},
            { "sName": 'total_tax', "mDataProp": 'total_tax', "mRender": prices_format},
            { "sName": 'total_discount' , "mDataProp": 'total_discount' , "mRender": prices_format},
            { "sName": 'shipping' , "mDataProp": 'shipping' , "mRender": prices_format},
            { "sName": 'total_due', "mDataProp": 'total_due', "mRender": prices_format},
                        { "sName": 'gl_status'   , "mDataProp": 'gl_status'   , "mRender": gl_status_format},
	        { "bSortable": false, "mRender": actions_format, "bSearchable": false }
        ]
    });
<?php if (!isset($disable_headings) || !$disable_headings): ?>
    <?php echo $invoices_table ?>.advancedSearch({
        aoColumns:[
            null,
...
<?php endif ?>
    ...
    <?php echo $invoices_table ?>.on("click", "a[href='#filter']", function(e){
        <?php echo $invoices_table ?>.prev(".datatable_filter").remove();
        if( filter == $(this).data("filter") && value == $(this).data("value") ){
            filter = undefined;
            value = undefined;
            <?php echo $invoices_table ?>._fnReDraw();
        }else{
            filter = $(this).data("filter");
            value = $(this).data("value");
            column = <?php echo $invoices_table ?>.find('thead th:eq('+$(this).parents("td").index()+')').text();
            text = $(this).data("text");
            <?php echo $invoices_table ?>._fnReDraw();
            $( "<div class='alert alert-info alert-sm datatable_filter'>"+
                ...
                column + " = " + text +
            "</span></p></div>" ).insertBefore(<?php echo $invoices_table ?>);
        }
    ...
    $(document).on('click', '.delete_selected', function() {
        if( !$(this).is('.disabled') ){
            ...
            bconfirm(globalLang['alert_confirmation'], function(){
                $('#<?php echo $invoices_table ?>').load_ajax(SITE_URL+"/invoices/delete", 'POST', selected_rows);
            });
        }
        return false;
    });
    ...
    $(document).on('click', '.print_selected', function() {
        if( !$(this).is('.disabled') ){
            ...
            MyWindow=window.open(SITE_URL+"/invoices/view?id="+selected_rows.join(","), WINDDOW_NAME, WINDDOW_CONFIGURATION);
        }
        return false;
    });
    ...
    $(document).on('click', '.pdf_selected', function() {
        if( !$(this).is('.disabled') ){
            ...
            location.href = SITE_URL+"/invoices/pdf?id="+selected_rows.join(",");
        }
        return false;
    });
    ...
    function actions_format(data, type, row, meta){
        actions = '<div class="btn-group">'+
                    '<a href="'+SITE_URL+"/invoices/open/"+row.id+'" class="btn btn-sm btn-secondary m-a-0" >'+
        ...
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("eye", SITE_URL+"/invoices/open/"+row.id, globalLang['details'], false);

        <?php if (!$this->ion_auth->in_group(array("customer", "supplier"))): ?>
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("trash", SITE_URL+"/invoices/delete?id="+row.id, globalLang['delete'], false, true, false, true);
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("pencil", SITE_URL+"/invoices/edit?id="+row.id, globalLang['edit'], false);
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("copy", SITE_URL+"/invoices/duplicate?id="+row.id, globalLang['duplicate'], false, true, false, true);

        actions += '<li class="dropdown-divider" ></li>';
        <?php endif ?>
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("print", SITE_URL+"/invoices/view?id="+row.id, globalLang['print'], true);
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("download", SITE_URL+"/invoices/pdf?id="+row.id, globalLang['tabletool_pdf'], false);
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("envelope", SITE_URL+"/invoices/email?id="+row.id, globalLang['email'], false, false, true);

        actions += '<li class="dropdown-divider" ></li>';
        if( row.total_due != 0 ){
            actions += $('#<?php echo $invoices_table ?>').create_datatable_action("money", SITE_URL+"/payments/create/"+row.id, globalLang['payments_create'], false, false, true);
        }
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("calendar-check-o", SITE_URL+"/payments/index/"+row.id, globalLang['payments'], false);
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("file-text-o", SITE_URL+"/receipts/index/"+row.id, globalLang['receipts'], false);
        <?php if ($this->ion_auth->is_admin()): ?>
        actions += $('#<?php echo $invoices_table ?>').create_datatable_action("history", SITE_URL+"/invoices/activities?id="+row.id, globalLang['activities'], false, false, true);
        <?php endif ?>
        actions += '</ul></div>';
    ...
    function reference_format(value, y ,row) {
        var html = '<a href="'+SITE_URL+'/invoices/open/'+row.id+'"><small class="font-weight-bold">'+value+'</small></a>';
        return  html;
    }
