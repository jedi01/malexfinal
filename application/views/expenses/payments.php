...
<ol class="breadcrumb">
    ...
    <div class="flip pull-right" style="line-height: 64px;">
        <?php if ($expense): ?>
        <a href="<?php echo site_url('/expenses/add_payment/'.$expense);?>" sis-modal="payments_table" class="btn btn-primary-outline sis_modal tip" title="<?php echo lang("payments_create"); ?>"> <i class="fa fa-plus"></i></a>
        <?php endif ?>
...
$(document).ready(function() {
    ...
    var payments_table = $('#payments_table').dataTable( {
        ...
        'bProcessing'    : true,
        'bServerSide'    : true,
        'sAjaxSource'    : SITE_URL+'/expenses/getPaymentsData',
        'fnServerData': function(sSource, aoData, fnCallback)
        {
            ...
            if( expense != undefined ){
                aoData.push( { "name": "expense_id", "value": expense } );
            }
            <?php
            if( isset($biller_id)){
        ...
        'fnDrawCallback': function(){
            $.each($(this).find('a.biller_popover'), function(i, biller_popover){
                $(biller_popover).popover({
                    ...
                    content:'<ul>'+
                    '<li class="dropdown-item"><a href="'+SITE_URL+'/suppliers/view?id='+$(biller_popover).data("supplier_id")+'" class="text-inherit sis_modal" sis-modal="payments_table"><i class="fa fa-expand"></i>'+globalLang["details"]+'</a></li>'+
        ...
        "aoColumns": [
            ...
            { "sName": 'expense_ref' , "mDataProp": 'expense_ref' , "mRender": reference_format},
    ...
    $(document).on('click', '.delete_selected', function() {
        if( !$(this).is('.disabled') ){
            ...
            bconfirm(globalLang['alert_confirmation'], function(){
                $('#payments_table').load_ajax(SITE_URL+"/expenses/delete_payment", 'POST', selected_rows);
            });
        }
        return false;
    });
    ...
    function actions_format(data, type, row, meta){
        ...
        actions += $('#payments_table').create_datatable_action("eye", SITE_URL+"/expenses/details_payment/"+row.id, globalLang['details'], false, false, true);
        actions += '<li class="dropdown-divider" ></li>';
        actions += $('#payments_table').create_datatable_action("trash", SITE_URL+"/expenses/delete_payment?id="+row.id, globalLang['delete'], false, true, false, true);
        actions += $('#payments_table').create_datatable_action("pencil", SITE_URL+"/expenses/edit_payment/"+row.id, globalLang['edit'], false, false, true);
        actions += '</ul></div>';
        return "<center>"+actions+"</center>";
    }
    function reference_format(value, y ,row) {
    var value = "<?php echo EXPENSE_PREFIX ?>"+String("00000" + value).slice(-5);
        var html = '<a href="'+SITE_URL+'/expenses/view/'+row.expense_id+'" sis-modal=""><small class="font-weight-bold">'+value+'</small></a>';
        return  html;
    }
    function number_format(data, type, row, meta) {
        var value = String("00000" + data).slice(-5);
        var html = '<a href="'+SITE_URL+'/expenses/details_payment/'+row.id+'" sis-modal="payments_table"><small class="font-weight-bold">'+value+'</small></a>';
        return  html;
    }
    ...
    function supplier_format(value, y ,row) {
        ...
        var html = "<a class='text-inherit biller_popover' data-toggle='popover' data-supplier_id='"+row.supplier_id+"' data-value='"+value+"' ><small>"+value+"</small></a>";
        return  html;
    }
